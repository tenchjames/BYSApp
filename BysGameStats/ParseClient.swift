...
class ParseClient {
    ...
    func getGamesByTeam(team: Team, completionHandler: (result: [PFObject]?, error: NSError?) -> Void) {
        let teamPointer = PFObject(className: "Team")
        teamPointer.objectId = team.objectId
        
        let homeGamesQuery = PFQuery(className: "Game")
        homeGamesQuery.whereKey("homeTeamPointer", equalTo: teamPointer)
        let awayGamesQuery = PFQuery(className: "Game")
        awayGamesQuery.whereKey("awayTeamPointer", equalTo: teamPointer)
        
        let query = PFQuery.orQueryWithSubqueries([homeGamesQuery, awayGamesQuery])
